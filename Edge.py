...
def get_6x6_h_blocks(ky, p, layer='top'):
    ...
    H_SOC_Ising = np.kron(0.5 * lambda_soc * soc_sign * Lz, sigma_z)
    h1 += H_SOC_Ising
...
N_width = 200  
ky_values = np.linspace(-np.pi, np.pi, 50)

bulk_bands = []
edge_bands = []

# 新增：用来记录最靠近 E=0 的边缘态的信息
best_edge_wavefunc = None
best_edge_E = float('inf')
best_edge_ky = 0

print("正在对角化 BdG 矩阵并寻找边缘态，请稍候...")
for ky in ky_values:
    H_bdg = build_bdg_matrix(ky, p, N_width)  # 注意：如果你用的是 s-wave，这里名字可能是 build_bdg_matrix_swave
    ...
    for j in range(len(evals)):
        ...
        if edge_weight > 0.6:
            ky_edge.append(E)
            # 【新增逻辑】：如果这个边缘态比之前找到的更靠近 E=0，就把它的波函数存下来！
            if abs(E) < abs(best_edge_E):
                best_edge_E = E
                best_edge_ky = ky
                best_edge_wavefunc = weight_per_cell.copy()  # 必须用 copy()，否则会被后续循环覆盖
        else:
            ky_bulk.append(E)
...
plt.title('Bilayer 2H-NbSe2 BdG Band Structure', fontsize=14)
