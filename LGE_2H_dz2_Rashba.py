...
class PauliMatrices_1:
    def __init__(self):
        ...
        self.tau_0 = np.zeros((6,6), dtype=complex)
        self.tau_0[0, 0] = 1
        self.tau_0[3, 3] = 1

        self.tau_z = np.zeros((6,6), dtype=complex)
        self.tau_z[0, 0] = 1
        self.tau_z[3, 3] = -1
...
class FileOutputSettings:
    ...
    def __init__(self, prefix="output", tag="run"):
        self.prefix: str = "./"     # 基础目录
        self.tag: str = "2H_dz2_Rashba"                      # 名称标签，可选
...
class Hamiltonian:
    ...
    def __init__(self, params):
        self.params = params
        self.pauli = PauliMatrices_1()

    def get_H_mono(self, k: np.ndarray) -> np.ndarray:
        ...
        H_R = np.kron(aR * np.eye(3), f_x * self.pauli.sigma_y - f_y * self.pauli.sigma_x) 

        H_mono_t = np.kron(H_TNN, self.pauli.sigma_0) + np.kron(H_soc, self.pauli.sigma_z) + H_R

        H_mono_b = np.kron(H_TNN, self.pauli.sigma_0) + np.kron(H_soc, self.pauli.sigma_z)
        
        return H_mono_t, H_mono_b
    ...
    def get_H_bilayer(self, k: np.ndarray) -> Tuple[np.ndarray]:
        ...
        H1, _ = self.get_H_mono(k)
        _, H2 = self.get_H_mono(-k)
        ...
        H_int_block = np.diag([h_dz2, h_dz2, h02, h02, h03, h03])
        ...
        H_final = H_bi + Eshift * np.eye(H_bi.shape[0])
...
class IrrepBasisFunctions:
    ...
    def __init__(self, pauli: PauliMatrices_1, para = Parameters):
        self.a = para.a
        self.pauli = pauli
    ...
    def _define_irrep_basis_functions(self) -> List[Dict]:
        ...
        irrep_0 = {
            ...
            'triplet': [
                    ...
                    lambda k, ff: np.kron(p.tau_0, (1j * (ff['s_k_minus'] * (p.sigma_x + 1j * p.sigma_y) - ff['s_k_plus'] * (p.sigma_x - 1j * p.sigma_y))/2) @ (1j * p.sigma_y)),

                    lambda k, ff: np.kron(p.tau_z, ff['s_k'] * p.sigma_z @ (1j * p.sigma_y)) ,

                    lambda k, ff: np.kron(p.tau_z, (1j * (ff['s_k_minus'] * (p.sigma_x + 1j * p.sigma_y) - ff['s_k_plus'] * (p.sigma_x - 1j * p.sigma_y))/2) @ (1j * p.sigma_y)),
                ]
            }
    ...
    def V_matrix(self, irrep_idx: int, v_vector: np.ndarray) -> np.ndarray:

        if irrep_idx == 0:

            v_vec_1=[v_vector[0],v_vector[1],v_vector[0],v_vector[1],v_vector[1],v_vector[1],v_vector[1],v_vector[1]]
            V = np.diag(v_vec_1)

        elif irrep_idx == 1:
            v_vec_0=[v_vector[1],v_vector[1]]
            V = np.diag(v_vec_0)

        elif irrep_idx == 2:
            V = np.kron(np.eye(16), v_vector[1])
...
class SuperconductorSolver:
    ...
    def __init__(self, params: Parameters, output: FileOutputSettings):
        ...
        self.hamiltonian = Hamiltonian(self.params)
        ...
        self._construct_V_matrix = self.irrep_basis.V_matrix
    ...
    def _setup_k_grid(self) -> np.ndarray:
            ...
            # 5. === [Layer 2] (Magitude Constraints) ===
            k_magnitude = np.sqrt(kx**2 + ky**2)
            
            #cond_mag_A = (k_magnitude >= 0.3) & (k_magnitude <= 0.65)
            
            #cond_mag_B = (k_magnitude > 0.75)
            
            mask_magnitude = k_magnitude >= 0.3
            ...
            for k in k_points_full:
                H_k = self.hamiltonian.get_H_bilayer(k)
    ...
    def _get_hamiltonian_h(self, k: np.ndarray) -> Tuple[np.ndarray]:
        ...
        H_mk = self.hamiltonian.get_H_bilayer(-k)
        H_mk = -H_mk.conj()
    ...
    def compute_phase_diagram(self) -> Dict:
        ...
        with open(dom_file, "w") as f_dom, open(tc_file, "w") as f_tc, open(chan_file, "w") as f_chan:
            f_dom.write("# v0           v1          Tc[K]   dom_irrep   deg\n")

            ir_labels = [ir["label"] for ir in self.irrep_basis.irrep_list]
            header_tc = " ".join([f"{lab:>8s}" for lab in ir_labels])
            f_tc.write(f"# {'v0':>10s} {'v1':>10s} " + header_tc + "\n")

            f_chan.write("# v0          v1          irrep  is_dominant  Tc[K]  deg_irrep  real[...] \n")
            ...
            for i, v0 in enumerate(p.v0_range):
                for j, v1 in enumerate(p.v1_range):
                    ...
                    # 即使 TcK_dom 是 None，也要写入文件（可选）
                    f_dom.write(f"{v0:.6f}  {v1:.6f}  ")
                    if TcK_dom is None:
                        f_dom.write("None       None        None\n")
                        # 跳过后续处理但保持 phase_data 的完整性
                    else:
                        ...
                        for ir_idx, TcK in enumerate(Tc_list_K):
                            if TcK is not None and abs(TcK - TcK_dom) < 1e-6:
                        ...
                        f_dom.write(
                            f"{TcK_dom:.6f}  {degenerate_str}  {dom_deg:d}\n"
                        )

                        # ========== 写入 Tc_all_irreps ==========
                        row_tc = f"{v0:10.6f}  {v1:10.6f}  "
                        for tc in Tc_list_K:
                            if tc is None:
                                row_tc += "     None "
                            else:
                        ...
                        for ir_idx, Tc in enumerate(res["irrep_Tcs"]):
                            if Tc is not None:
                                if ir_idx not in irrep_eigenvecs:
                                    _, vecs = self._solve_irrep_eigensystem(ir_idx, Tc, vvec, return_vectors=True)
                                    irrep_eigenvecs[ir_idx] = vecs
                        ...
                        for ir_idx, Tc in enumerate(res["irrep_Tcs"]):
                            ...
                            eigvecs = irrep_eigenvecs[ir_idx]
                            ...
                            for i_vec, vec in enumerate(eigvecs):
                                ...
                                f_chan.write(
                                    f"{v0:10.6f}  {v1:10.6f}  {label:6s}   {is_dom}  {TcK:8.6f}  {deg_count:2d}  "
                                    f"[{real_s}]\n"
                                )
